...
class ExtractorTab:
    def __init__(self, parent, main_app=None):
        ...
        if main_app and hasattr(main_app, 'unrar_after_download'):
            ...
            self.unrar_after_download = tk.BooleanVar(value=True)  # Auto-unrar option - ENABLED by default
    ...
    def build_ui(self):
        ...
        # Password section
        ttk.Label(main_container, text="Password:", font=("Segoe UI", 10)).grid(row=3, column=0, sticky="w", padx=(0, 10))
        password_frame = ttk.Frame(main_container)
        password_frame.grid(row=3, column=1, sticky="ew", pady=(0, 15))
        
        password_entry = ttk.Entry(password_frame, textvariable=self.password, width=15, font=("Segoe UI", 9), show="*")
        password_entry.pack(side="left", fill="x", expand=False)
        
        # Show/Hide password toggle button
        self.show_password_btn = ttk.Button(password_frame, text="👁️", command=self.toggle_password_visibility, width=3)
        self.show_password_btn.pack(side="left", padx=(5, 0))
        
        # Store password entry reference for toggle method
        self.password_entry = password_entry
        
        # Tool configuration section
        self.tool_frame = ttk.Frame(main_container)
        self.tool_frame.grid(row=4, column=0, columnspan=2, sticky="ew", pady=(0, 15))
        ...
        progress_frame.grid(row=6, column=0, columnspan=2, sticky="ew", pady=(0, 15))
...
• C:\Program Files (x86)\WinRAR\WinRAR.exe"""
    ...
    def open_smart_manager(self, auto_start=False):
        ...
        if hasattr(self.smart_manager, 'delete_after_extract'):
    ...
    def check_downloads_and_extract(self):
        ...
        print("DEBUG: check_downloads_and_extract() called!")
        print(f"DEBUG: Auto-extract enabled: {self.unrar_after_download.get()}")
        print(f"DEBUG: Main app exists: {bool(self.main_app)}")
        
        if not self.unrar_after_download.get() or not self.main_app:
            print("DEBUG: Returning early - auto-extract disabled or no main app")
            return
        
        try:
            from config_manager import get_archive_groups_status, get_group_fetch_total, get_url_tracking
            import os
            from pathlib import Path
            
            groups = get_archive_groups_status()
            url_tracking = get_url_tracking()
            download_folder = Path(self.main_app.download_dir) if self.main_app else Path("downloads")
            
            print(f"DEBUG: Found {len(groups)} archive groups")
            ...
            for group_key, group_data in groups.items():
                group_name = group_data.get('base_name', group_key.replace('_group', ''))
                downloaded_count = group_data.get('downloaded_count', 0)
                imported_parts = group_data.get('imported_parts', [])
                
                print(f"DEBUG: Checking group '{group_name}' (key: {group_key})")
                print(f"DEBUG:  Downloaded count: {downloaded_count}")
                print(f"DEBUG:  Imported parts: {imported_parts}")
                ...
                if download_folder.exists():
                    for file in download_folder.iterdir():
                        if file.is_file():
                            ...
                            base_patterns = [
                                f"{group_name.lower()}.part",
                                f"{group_name.lower()}_part",
                                f"{group_name.lower()}.",
                ...
                print(f"DEBUG:  Files in download folder: {download_folder_count}")
                print(f"DEBUG:  Download folder files: {download_folder_files}")
                
                # Method 2: Get fetch total from stored data
                fetch_total = get_group_fetch_total(group_name)
                print(f"DEBUG:  Fetch total: {fetch_total}")
                
                # Method 3: Get imported count from JSON
                imported_count = len(imported_parts)
                print(f"DEBUG:  Imported count: {imported_count}")
                
                # Determine the most reliable total
                if fetch_total > 0:
                    total_parts = fetch_total
                    source = "fetch data"
                elif imported_count > 0:
                    total_parts = imported_count
                    source = "imported data"
                else:
                    total_parts = download_folder_count
                    source = "download folder"
                
                print(f"DEBUG:  Using total_parts={total_parts} from {source}")
                print(f"DEBUG:  Completion: {downloaded_count}/{total_parts} ({(downloaded_count/total_parts*100) if total_parts > 0 else 0:.1f}%)")
                
                # Check if group is complete using the most reliable total
                is_complete = False
                if total_parts > 0 and downloaded_count >= total_parts:
                    is_complete = True
                    print(f"DEBUG:  Group '{group_name}' is COMPLETE!")
                else:
                    missing_count = total_parts - downloaded_count
                    print(f"DEBUG:  Group '{group_name}' is INCOMPLETE - missing {missing_count} files")
                
                if is_complete:
                    complete_groups.append(group_key)
                    print(f"DEBUG: Group '{group_key}' is complete and eligible for extraction")
                else:
                    incomplete_groups.append(group_key)
                    print(f"DEBUG: Group '{group_key}' not complete yet")
            
            print(f"DEBUG: Complete groups: {complete_groups}")
            print(f"DEBUG: Incomplete groups: {incomplete_groups}")
            
            # Extract complete groups immediately (don't wait for all groups)
            if complete_groups:
                print(f"DEBUG: Found {len(complete_groups)} complete groups - triggering Smart Folder Manager")
                ...
                monitoring_msg = f"🔍 Monitoring {len(incomplete_groups)} incomplete groups..."
                for group_key in incomplete_groups:
                    group_data = groups[group_key]
                    group_name = group_data.get('base_name', group_key.replace('_group', ''))
                    downloaded = group_data.get('downloaded_count', 0)
                    total = group_data.get('total_parts', 0)
                    monitoring_msg += f"\n• {group_name}: {downloaded}/{total}"
                
                self.auto_status_label.config(text=monitoring_msg, foreground="orange")
